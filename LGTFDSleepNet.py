...
class MultiSpectralAttentionLayer1D(nn.Module):
    def __init__(self, channel, length, reduction=16, freq_sel_method='top16'):
        ...
        self.mapper_bot = get_freq_indices('mid' + freq_sel_method[3:])
...
class FcaBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        c2l = dict([(29, 384), (128, 28), (128*3, 29)]) 
        self.planes = planes
...
class LGTFDSleepNet(nn.Module):
    
    def __init__(self, config):
        super(LGTFDSleepNet, self).__init__()
        ...
        self.global_pool_time = nn.Linear(384,  2)
    ...
    def forward(self, x):
        ...
        x1 = self.transformer_encoder_1(x1)     # (batch_size, 29, 128), (batch, time, frequency)
        ...
        x = x.view(x.size(0), -1) + x_au_residual 
        x = self.fc1(x)
        x = self.fc2(x)
        return x, x_au